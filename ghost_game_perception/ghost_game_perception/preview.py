...
def main(argv=None):
    ...
    if not args.model_path.expanduser().is_file():
        parser.error("Model missing. Run python -m ghost_game_perception.download_model first.")
    ...
    try:
        ...
        parser.exit(1, "OpenCV missing. Install ghost_game_perception/requirements.txt.\n")
    capture = None
    model = None
    try:
